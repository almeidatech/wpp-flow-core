...
4 MCPs configured for Claude Code:

| MCP | Tools Available | Use Case |
|-----|-----------------|----------|
| **n8n-mcp** | 1,084 nodes, 2,709 templates | Workflow automation, node lookup |
| **memory** | Session context | Persistent memory across conversations |
| **github** | PR, Issues, Releases | Repository management (delegated to @devops) |
| **postgres** | Query, Schema | Chatwoot database (direct queries for analysis) |

**Tool Selection Priority:**
1. **Native tools first** (Read, Write, Bash, Glob, Grep) - fastest, local execution
2. **MCP for external services** - when native tools insufficient
3. **docker-gateway for isolation** - complex operations, container isolation

## Custom Agents for Domain Specificity

8 domain-specific agents installed in `.claude/agents/`:
...
| Skill | Purpose | Usage |
|-------|---------|-------|
| `skill-creator` | Create custom Claude Code skills | `@aios-master *create-skill` |
...
**Critical Rule:** Use native Claude Code tools FIRST for local operations:

| Task | Use This | NOT This |
|------|----------|----------|
| Read files | `Read` tool | MCP |
| Write files | `Write` / `Edit` tools | MCP |
| Run commands | `Bash` tool | MCP |
| Search files | `Glob` / `Grep` | MCP |

MCP servers (n8n-mcp, postgres-mcp) are for:
- Querying external services
- Workflows requiring external data
- Complex integrations

**DevOps Exception:** @devops has exclusive authority for GitHub operations (push, PR, releases).
...
@sm *create-next-story                 # Create detailed story
@dev                                    # Switch to dev agent
*help                                   # See available tasks
...
@devops *push                           # Push changes
@devops *create-pr                      # Create PR
@devops *release                        # Create versioned release

# Build n8n automation
@javascript-pro                         # For n8n code
@nlp-engineer                           # For AI/chatbot features
@customer-support                       # For support workflows
```
